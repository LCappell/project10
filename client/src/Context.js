...
// Create context variable to store required context
...
export class Provider extends Component {
  ...
  // Signs user in - allowed to edit and update courses
  ...
  signUp = async (user) => {
    ...
    if (newUser.status === 400) {
      ...
      //Sign in function
      this.signIn(user.emailAddress, user.password).then((data) => {
        this.setState(() => {
          return {
  ...
  updateCourse = async (courseData) => {
    ...
    if (
      formattedAuth.emailAddress === this.state.currentCourseOwner.emailAddress
    ) {
      const updatedData = {
      ...
      const response = await this.data.updateCourse(
        this.state.currentCourseInfo.id,
        updatedData,
        formattedAuth
      );
      return response;
